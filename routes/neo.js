...
router.get('/nequery1',function(req, res){
...
router.get('/neoquery14',function(req, res){
	//Writeyour neo4j  query
	var res14 =[];
	db("MATCH (fact:Fact), (time:time_dimension) WHERE fact.time_id=time.time_id RETURN DISTINCT time.year AS label, COUNT(fact.game_id) AS value ORDER BY label").on('data', function (result){res14.push(result);
